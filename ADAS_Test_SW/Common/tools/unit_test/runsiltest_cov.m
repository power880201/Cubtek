function runsiltest_cov(xlsfile,model,resfile,verbose_flag)
...
% 
% $Id: runsiltest_cov.m 2 2019-09-19 09:28:50Z JuneXmas $
%
...
for i=1:length(sheetnames)
    ...
    [result_struct,failed_to_run,cvdo] = run_test_on_model(model,input_struct);
    if i==1
        cvdo_final=cvdo;
    else
        cvdo_final=cvdo_final+cvdo;
    end
    if failed_to_run
        return;
    end
...
if written
    [~, f, x]=fileparts(resfile);
    hyperlink = sprintf('<a href="matlab:system(''start %s'');">%s%s</a>', resfile, f, x);
    fprintf('  (Results have been written to %s)\n',hyperlink);
    path=[fileparts(which(model)) '\Test_Report\' upper(strtok(model,'_')) '_CoverageReport\'];
    if exist(path,'dir')==7
    else
        mkdir(path)        
    end
    covReport_name=['Test_Report\' upper(strtok(model,'_')) '_CoverageReport\' upper(strtok(model,'_')) '_CoverageReport.html'];
    cvhtml(covReport_name, cvdo_final,'-sRT=0');
    cvsave_name=[upper(strtok(model,'_')) '_CoverageResults'];
    cvsave(cvsave_name,cvdo_final);
else
...
%==========================================================================
function [tframe,failed_to_run,cvdo] = run_test_on_model(model,test_structure)
    ...
    if run_verbose
        ...
        system=strcat(model,'/',upper(strtok(model,'_')));
        cov_obj=cvtest(model);
        cov_obj.settings.decision=1;
        cov_obj.settings.condition=1;
        cov_obj.settings.mcdc=1;
        cov_obj.settings.tableExec=0;
        cov_obj.settings.sigrange=0;
        cov_obj.settings.sigsize=0;
        cov_obj.settings.designverifier=0;
        cov_obj.settings.overflowsaturation=0;
        cov_obj.settings.relationalop=0;
        cov_obj.modelRefSettings.enable='on';
        [cvdo,~] = cvsim(cov_obj,...
