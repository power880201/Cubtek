/*
 * TestFrameInput_mk2_sf.c
 *
 * $Id: TestFrameInput_ovride_sf.c 2 2019-09-19 09:28:50Z JuneXmas $
 */

#define S_FUNCTION_NAME  TestFrameInput_ovride_sf
...
static void mdlOutputs(SimStruct *S, int_T tid)
{
	...
	/* Keep the output values in synch with timesteps array. */
	if(it < nt && t >= pt[it])
	{
		ssSetRWorkValue(S,0,pv[it]); /* set a new current value. */
		ssSetIWorkValue(S, 1, ++it);
	}

	if(u[0] >= 0.5)
	{
		/* play the current input value. */
		y[0] = u[1]; /* output the current value. */
	}
	else
	{
