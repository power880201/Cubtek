...
#define S_FUNCTION_NAME        RTICANMM_SETUP_Vehicle_CAN                                                                                          
...
#include "RTICANMM_SETUP_Vehicle_CAN.h"                                                                                                    
...
static Float64 CANMM_CTL_SETUP_Vehicle_CAN_baudrate      = 500.0;                                                                                 
static Float64 CANMM_CTL_SETUP_Vehicle_CAN_baudrate_in   = 500.0; /* Temporary for input port value */                                            
static Float64 CANMM_CTL_SETUP_Vehicle_CAN_baudrate_old  = 500.0;                                                                                 
...
volatile UInt8 CANMM_CTL_SETUP_Vehicle_CAN_ID;                                                                                              
volatile UInt8 CANMM_CTL_SETUP_Vehicle_CAN_TX_STATUS                 = 1;                                                                   
volatile UInt8 DECLSPEC CANMM_CTL_SETUP_Vehicle_CAN_Variation        = 1;                                                                   
volatile UInt8 DECLSPEC CANMM_CTL_SETUP_Vehicle_CAN_VariationSwitch  = 0;                                                                   
...
#endif                                                                                                           
volatile UInt8   CANMMMonitor_Vehicle_CAN     = 0;                                                                                     
volatile UInt8   CANMMMonitor_Vehicle_CAN_old = 0;                                                                                     
volatile dsfloat MonitoringTimeStamp_Vehicle_CAN = 0.0;                                                                                       
...
static CANMM_CTL_CLIENT_TYPE(CANMM_CTL_SETUP_Vehicle_CAN) ctlsetup_client;                                                                
#endif /* RTICANMM_DSDAQ_SERVICEID */                                                                            
                                                                                                                 
                                                                                                                   
void                                                                                                               
CANMM_CTL_SETUP_Vehicle_CAN_baudrate_set(Float64 baudrate) {                                                                                
  ...
  if ((Error = DsCanCh_apply(CanMM_channel)) != DSCAN_NO_ERROR)                                                    
    ...
    CANMM_CTL_SETUP_Vehicle_CAN_baudrate = baudrate;                                                                                        
  }                                                                                                                
}                                                                                                                  
...
void DECLSPEC CANMM_CTL_SETUP_Vehicle_CAN_client_init(CANMM_CTL_SETUP_Vehicle_CAN_CtlClient* client) {                                                               
  ...
  if (!client->initialized) {                                                                                      
    ...
    if ((Error = DsCanMsg_setQueueSize(client->tx_msg, CANMM_CTL_SETUP_Vehicle_CAN_tx_queue_size)) != DSCAN_NO_ERROR)                       
      CANMM_DEBUG_ERROR(Error);                                                                                    
#if (defined(DSCAN_VER_MAR) && DSCAN_VER_MAR == 1) || defined(RTICANMM_HAVE_DSRT)                                  
 ...
 * If CANMM_CTL_SETUP_Vehicle_CAN_rx_stop has been issued before, the receive buffer (mcr) is cleared                                       
 ...
 * CANMM_CTL_SETUP_Vehicle_CAN_rx function. To continue reception of messages CANMM_CTL_SETUP_Vehicle_CAN_rx_start                                                   
 * has to be called.                                                                                               
 */                                                                                                                
...
Float64 DECLSPEC CANMM_CTL_SETUP_Vehicle_CAN_baudrate_get() {                                                                               
  return CANMM_CTL_SETUP_Vehicle_CAN_baudrate;                                                                                              
}                                                                                                                  
...
UInt32 DECLSPEC CANMM_CTL_SETUP_Vehicle_CAN_tx_queue_level(CANMM_CTL_SETUP_Vehicle_CAN_CtlClient* client) {                                                          
  ...
  if ((Error = DsCanMsg_getTxQueueCount(client->tx_msg, &tx_count)) != DSCAN_NO_ERROR) {                           
    ...
    tx_count = CANMM_CTL_SETUP_Vehicle_CAN_tx_queue_size;                                                                                   
  }                                                                                                                
  return tx_count;                                                                                                 
}                                                                                                                  
...
# endif /* RTICANMM_HAVE_DSCAN_BUS */                                                                              
  ...
  if ((Error = DsCanCh_setDescriptor(CanMM_channel, "Vehicle_CAN")) != DSCAN_NO_ERROR)                                      
    CANMM_DEBUG_ERROR(Error);                                                                                      
                                                                                                                   
  if ((Error = DsCanCh_setBaudRate(CanMM_channel, (int)CANMM_CTL_SETUP_Vehicle_CAN_baudrate*1000)) != DSCAN_NO_ERROR)                   
      CANMM_DEBUG_ERROR(Error);                                                                                
...
# endif                                                                                                            
#if defined(RTICANMM_DSDAQ_SERVICEID)																				  
    CANMM_CTL_CLIENT_INIT(CANMM_CTL_SETUP_Vehicle_CAN, &ctlsetup_client);                                                                   
    CANMM_CTL_RX_STOP(CANMM_CTL_SETUP_Vehicle_CAN, &ctlsetup_client);      /* Start disabled */                                             
# endif                                                                                                            
...
#if defined(RTICANMM_DSDAQ_SERVICEID)                                                                            
if (CANMMMonitor_Vehicle_CAN > 0 && !(CANMMMonitor_Vehicle_CAN_old > 0)) { /* monitoring enabled */                                                          
  CANMM_CTL_RX_START(CANMM_CTL_SETUP_Vehicle_CAN, &ctlsetup_client);                                                                      
}                                                                                                                
if (!(CANMMMonitor_Vehicle_CAN > 0) && CANMMMonitor_Vehicle_CAN_old > 0) { /* monitoring disabled */                                                         
  CANMM_CTL_RX_STOP(CANMM_CTL_SETUP_Vehicle_CAN, &ctlsetup_client);                                                                       
}                                                                                                                
CANMMMonitor_Vehicle_CAN_old = CANMMMonitor_Vehicle_CAN;                                                                                                     
                                                                                                                 
if (CANMMMonitor_Vehicle_CAN > 0) {                                                                                                    
  while (CANMM_CTL_RX(CANMM_CTL_SETUP_Vehicle_CAN, &ctlsetup_client, &canmm_msg)) {                                                       
    UInt32 i = 0;                                                                                                
    int j = 0, k = 0;                                                                                            
...
#endif                                                                                                           
    MonitoringTimeStamp_Vehicle_CAN = canmm_msg.TimeStamp;                                                                                    
  ...
  if (CANMM_CTL_SETUP_Vehicle_CAN_VariationSwitch == 0) {                                                                                   
    CANMM_CTL_SETUP_Vehicle_CAN_ID = INDEF_VARIATION;                                                                                       
  }                                                                                                                
  else {                                                                                                           
    CANMM_CTL_SETUP_Vehicle_CAN_ID = CANMM_CTL_SETUP_Vehicle_CAN_Variation;                                                                                          
  }                                                                                                                
...
/****************************/                                                                                     
  ...
  CANMMMonitor_Vehicle_CAN_old = 0;                                                                                                      
