/*****************************************************************************************************************************/
/* MAIN S-FUNCTION for RTICANMM MainBlock (RTICANMM_MAIN_Vehicle_CAN.c) */
/*                                                                                                                           */
/*   AUTHOR(S):                                                                                                              */
/*      U. Homann                                                                                                              */
/*      M. Eikermann                                                                                                              */
...
#include "RTICANMM_MAIN_Vehicle_CAN_Globals.h"                                                                                                              
...
#define CANMM_CONTROLLER_ID CANMM_CTL_SETUP_Vehicle_CAN                                                                                             
...
{                                                                                                                      
  ...
  static Int32 StartIdx[8] = {0, 32, 32, 32, 32, 32, 32, 32};                                                         
  static Int32 StopIdx[8]  = {31, 31, 31, 31, 31, 31, 31, 31};                                                         
  static Int32 NumOfIds[8] = {32, 0, 0, 0, 0, 0, 0, 0};                                                         
  Int32 MsgId = Msg->Id;                                                                                                     
  ...
  for (i=0; i<8; i++) {                                                                                            
    if( NumOfIds[MsgIDFormat]>0) {                                                                                 
      ...
      while (!found && (Start <= Stop)) {                                                                          
        ...
        Pos = (Start + Stop)>>1;                                                                                   
        if ((ID_Table_RX(Pos) == MsgId) && (Msg->Format == CANMMVehicle_CAN_MIDFRX[Pos])) {                                             
          found = 1;                                                                                               
          break; // while                                                                                          
        }                                                                                                          
        else {                                                                                                     
...
#include CAN_MESSAGE_HEADERFILE                                                                                            
  ...
  {                                                                                                                        
      ...
      {                                                                                                                
          msg_error_printf(MSG_SM_CANNEDY, RTICANMM_MAINSFCN_MALLOC_ERROR , RTICANMM_MAINSFCN_MALLOC_ERROR_TXT, "Vehicle_CAN");
      }                                                                                                                
  }                                                                                                                        
  for (i = 0; i < NUM_TX_MSG; i++) {                                                                                   
...
{                                                                                                                          
  ...
  void **PWork = ssGetPWork(S);                                                                                            
      ...
      msg_info_printf(MSG_SM_CANNEDY, RTICANMM_MAINSFCN_MDL_START_INFO , RTICANMM_MAINSFCN_MDL_START_INFO_TXT, "Vehicle_CAN");  
#endif                                                                                                                     
  ...
  msg_info_printf(MSG_SM_CANNEDY, RTICANMM_MAINSFCN_MDL_START_CYCLETIME_INFO, RTICANMM_MAINSFCN_MDL_START_CYCLETIME_INFO_TXT, "Vehicle_CAN", TX_CYCLETIME[0]);
#endif                                                                                                                 
    ...
    while (CANMM_CTL_RX(CANMM_CONTROLLER_ID, &ctlsetup_client, &MsgData.CANMsg)) {                                            
      ...
      if(!MsgData.CANMsg.Loopback) {                                                                                       
        msg_info_printf(MSG_SM_CANNEDY, RTICANMM_MAINSFCN_RX_LOOPBACK_INFO,                                                
                        RTICANMM_MAINSFCN_RX_LOOPBACK_INFO_TXT, "Vehicle_CAN",                                                      
                        MsgData.CANMsg.Id,                                                                                 
        ...
        msg_info_printf(MSG_SM_CANNEDY, RTICANMM_MAINSFCN_RX_MSG_INFO, RTICANMM_MAINSFCN_RX_MSG_INFO_TXT, "Vehicle_CAN",            
                        MsgData.CANMsg.Id,                                                                                 
                        MsgData.CANMsg.TimeStamp,                                                                          
                        MsgData.CANMsg.Dlc,                                                                                
                        MsgData.CANMsg.Data[0]);                                                                           
      ...
      if (idIdx != -1) {                                                                                           
        ...
        CANMMVehicle_CAN_RXt[idIdx] = MsgData.CANMsg.TimeStamp;                   
        if (MsgData.CANMsg.Loopback) {                          
          CANMMVehicle_CAN_RXdt[idIdx] = MsgData.CANMsg.TimeStamp - TX_OLDTIME[idIdx];
          TX_OLDTIME[idIdx] = MsgData.CANMsg.TimeStamp;         
          CANMMVehicle_CAN_RXsta[idIdx] = 1;                                    
        }                                                       
        else {                                                  
          CANMMVehicle_CAN_RXdt[idIdx] = MsgData.CANMsg.TimeStamp - RX_OLDTIME[idIdx];
...
/* Set status of message (if message could basically be sent) -------------------------- */                                                            
     ...
     {                                                                                                                  
      ...
      j = 0;                                                                                                           
      i = 0;                                                                                                           
      if (CANMMVehicle_CAN_MAXMSGPERSTEP < 1) {                                                                                                    
          if (MAX_TX_PERSTEP < 1) {                                                                                    
            CANMMVehicle_CAN_MAXMSGPERSTEP = 1;                                                                                                    
          }                                                                                                            
          else {                                                                                                       
            CANMMVehicle_CAN_MAXMSGPERSTEP = MAX_TX_PERSTEP;                                                                                       
          }                                                                                                            
      }                                                                                                                
                                                                                                                       
      do { /* while ((i < NUM_TX_MSG) && (j < (CANMMVehicle_CAN_MAXMSGPERSTEP-(Int32)tx_queue_level)) && (j < CANMM_CTL_TX_QUEUE_SIZE(CANMM_CONTROLLER_ID))) */           
        UInt32 ID_number = TX_QUEUE(i).id;                                                                                 
        if (ID_number != NOCANID) {                                                                                        
          UInt8 TxMsgSent = 0;                                                                                             
                                                                                                                           
#if RTICANMM_LOGLEVEL > 3                                                                                                  
          msg_info_printf(MSG_SM_CANNEDY, RTICANMM_MAINSFCN_TX_MSG_INFO, RTICANMM_MAINSFCN_TX_MSG_INFO_TXT, "Vehicle_CAN",  ID_number);
#endif                                                                                                                     
                                                                                                                           
          switch (CANMMVehicle_CAN_RXMT[ID_Table_TX(i)]) {                                                                                    
        ...
        i++;                                                                                                               
      } while ((i < NUM_TX_MSG) && (j < (CANMMVehicle_CAN_MAXMSGPERSTEP-(Int32)tx_queue_level)) && (j < CANMM_CTL_TX_QUEUE_SIZE(CANMM_CONTROLLER_ID)));       
    }                                                                                                                  
